...
3-band Filter-bank Riemannian Tangent Space + L2 Logistic Regression,
v1.2 intermediate decision layer, covariance-estimator ablation (Empirical/SCM).

Purpose
-------
This is a strictly controlled covariance-estimator variant of the current
32-channel TS+LR v1.2 baseline. Data loading, channel layout, frequency bands,
training window, L2 Logistic Regression, out-of-fold sigmoid calibration,
nested leave-one-run-out evaluation, and pseudo-online decision layer are kept
unchanged. Only the covariance estimator is set to:

    COV_ESTIMATOR = "scm"  # Empirical/SCM
...
- Channel layout: simulated 32-channel company-device layout
- Filter bank: 8-13, 13-20, 20-30 Hz
- Feature/model: Empirical/SCM covariance -> Riemannian Tangent Space -> L2 LR
- Classifier: LogisticRegression, L2 penalty, C=1.0, balanced class weights
...
- Pseudo-online decision layer: v1.2 intermediate

Device-channel simulation
-------------------------
The company device labels FT9/FT10/TP9/TP10. EEGBCI does not provide those exact
channels, so the nearest available neighbors are used:
...
Suggested folder: D:\BCI\FBCSPNet\June\June8-12\04

Run
---
python -u pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_same_folder.py > scm_95subjects_run.log 2>&1

Expected outputs beside this script
-----------------------------------
pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_results.json
pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_results_selected_configs.csv
pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_subject<ID>.json
...
# Riemannian + fixed L2 Logistic Regression settings
# Controlled ablation: this is the only substantive setting changed from OAS.
COV_ESTIMATOR = "scm"  # Empirical/SCM
TANGENT_METRIC = "riemann"

# One-element lists retain the original nested-search code structure without
# introducing an additional classifier hyperparameter search.
LR_C_LIST = [1.0]
LR_L1_RATIO_LIST = [0.0]  # structural metadata only; penalty is explicitly L2
LR_MAX_ITER = 1000
LR_TOL = 1e-4
LR_ZERO_TOL = 1e-8
...
class FilterBankRiemannianTSLR:
    """Filter-bank covariance -> tangent-space features -> L2 LR + optional sigmoid calibration."""
    ...
    def __init__(
        ...
        self.l1_ratio = float(l1_ratio)  # metadata placeholder; not passed to L2 LR
    ...
    def _fit_oof_sigmoid_calibrator(self, X_by_band, y):
        ...
        cal_meta = {
            ...
            "input": "base_l2_lr_decision_function",
            "cv": int(n_splits),
    ...
    def fit(self, X_by_band, y):
        ...
        self.training_summary_ = {
            ...
            "classifier": "L2_LogisticRegression",
            "lr_penalty": "l2",
            "lr_solver": "lbfgs",
...
def run_one_subject(data_dir, subject_id):
    print("=" * 80)
    print(f"Pseudo-online 3-band FilterBank Riemannian TS+LR v1 | subject {subject_id}")
    print(f"RUNS               = {RUNS}")
    print(f"BANDS              = {BANDS}")
    ...
    print(f"LR_C_LIST          = {LR_C_LIST}")
    print(f"L2_PLACEHOLDER     = {LR_L1_RATIO_LIST}")
    ...
    fold_search_space = {
        ...
        "lr_c_list": LR_C_LIST,
        "lr_l1_ratio_placeholder_list": LR_L1_RATIO_LIST,
    ...
    for test_run in RUNS:
        ...
        for val_run in dev_runs:
            ...
            for tmin, tmax in fold_search_space["train_windows"]:
                ...
                for model_idx, (lr_c, l1_ratio) in enumerate(itertools.product(
                    fold_search_space["lr_c_list"],
                    fold_search_space["lr_l1_ratio_placeholder_list"],
                )):
                    seed = base_seed + model_idx
                    print(
                        f"Training FilterBank Riemannian TS+LR (scm) | subj={subject_id} "
        ...
        print(
            f"Training FINAL FilterBank Riemannian TS+LR (scm) | subj={subject_id} "
        ...
        fold_summary = {
            ...
            "selection_status": f"loro_inner_run_validation_filterbank_riemannian_ts_lr_v1_2_intermediate_scm_3bands::{selection_meta['selection_mode']}",
            "best_validation_config": {
                ...
                "probability_output": "cv_sigmoid_calibrated_l2_lr_decision_function" if USE_SIGMOID_CALIBRATION else "l2_lr_predict_proba_no_extra_calibration",
...
def main():
    ...
    for subject_id in SUBJECT_IDS:
        ...
        subj_json = output_dir / f"pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_subject{subject_id}.json"
    ...
    overall = {
        "model": "PseudoOnline_FilterBank3Bands_32ChDevice_Riemannian_TangentSpace_LR_SCMCov_MIvsRest_LORO_V1_2Intermediate_95Subjects",
    ...
    overall["selection_policy"] = {
        "strategy": "balanced_robust_filterbank3bands_riemannian_ts_lr_v1_2_intermediate_scm",
        "target_max_inner_val_far": TARGET_MAX_INNER_VAL_FAR,
        "target_max_inner_val_far_std": TARGET_MAX_INNER_VAL_FAR_STD,
        "target_min_inner_val_hit": TARGET_MIN_INNER_VAL_HIT,
        "fixed_refractory_sec": FIXED_REFRACTORY_SEC,
        ...
        "probability_calibration": {
            ...
            "input": "base L2 LR decision_function score",
    ...
    out_json = output_dir / f"pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_{suffix}.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(overall, f, indent=2, ensure_ascii=False)

    out_csv = output_dir / f"pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_{suffix}_selected_configs.csv"
    if csv_rows:
    ...
    if subject_errors:
        out_err = output_dir / f"pseudo_online_filterbank3bands_32ch_device_riemannian_ts_lr_v1_2_intermediate_scm_95subjects_{suffix}_subject_errors.json"
    ...
    print("=" * 80, flush=True)
    print("Finished 3-band 32-channel device-layout TS+LR v1.2 Empirical/SCM covariance 95-subject benchmark", flush=True)
