...
10 subjects, expanded window/step search, unconstrained selection.

Protocol:
- Outer loop: leave one run out as held-out test run.
- Inner run-based validation on the remaining two runs:
    * train on one dev run, validate on the other
    * swap roles and repeat
- Aggregate validation metrics across the two inner run splits
- Select the best configuration from aggregated validation performance
- Retrain on both dev runs together with the selected config
- Evaluate once on the held-out test run

This is stricter than random trial train/val/test split because the entire test run
is unseen during training and parameter selection.
"""
...
def evaluate_decision_layer_from_base_traces(base_traces, smooth_k, threshold, min_consecutive, include_trial_traces=False):
    ...
    for rec in base_traces:
        ...
        detected, first_detect_idx = detect_from_probs(smoothed, threshold=threshold, min_consecutive=min_consecutive)
        pred_event = 1 if detected else 0
        true_label = int(rec["true_label"])

        if detected and first_detect_idx is not None:
            det_epoch = float(rec["times_epoch_sec"][first_detect_idx])
            det_cue = float(rec["times_cue_sec"][first_detect_idx])
        else:
            det_epoch = None
            det_cue = None
    ...
    source = trial_results if include_trial_traces else None
    if source is None:
        for rec, truth in zip(base_traces, event_truth):
            ...
            detected, first_detect_idx = detect_from_probs(smoothed, threshold=threshold, min_consecutive=min_consecutive)
            if detected and first_detect_idx is not None:
                detection_latencies_epoch.append(float(rec["times_epoch_sec"][first_detect_idx]))
                detection_latencies_cue.append(float(rec["times_cue_sec"][first_detect_idx]))
    else:
        for tr in source:
...
def config_to_string(d):
    return (
        ...
        f"thr={d['threshold']}|min_consec={d['min_consecutive_windows']}"
    )
...
def aggregate_inner_results(inner_results):
    ...
    key_fields = [
        ...
        "smooth_k", "threshold", "min_consecutive_windows"
    ]
...
def run_one_subject(data_dir, subject_id):
    print("=" * 80)
    print(f"Pseudo-online LORO expanded-search | subject {subject_id}")
    print(f"RUNS           = {RUNS}")
    ...
    # Per-run cache: (run_id, tmin, tmax) -> (X, y, sfreq)
    epoch_cache = {}
    run_trial_counts = {}
    run_label_counts = {}
    sfreq_ref = None
    ...
    for test_run in RUNS:
        ...
        inner_results = []
        # Inner run-based validation: swap the two dev runs.
            ...
            for tmin, tmax in TRAIN_WINDOWS:
                ...
                for win_sec, step_sec in itertools.product(WIN_SEC_LIST, STEP_SEC_LIST):
                    ...
                    for smooth_k, threshold, min_consecutive in itertools.product(
                        SMOOTH_K_LIST, THRESHOLD_LIST, MIN_CONSECUTIVE_LIST
                    ):
        ...
        fold_summary = {
            ...
            "selection_status": "loro_inner_run_validation",
...
def main():
    ...
    for subject_id in SUBJECT_IDS:
        ...
        subj_json = output_dir / f"pseudo_online_loro_expanded_search_subject{subject_id}.json"
    ...
    overall = {
        "model": "PseudoOnline_Riemannian_TangentSpace_LR_MIvsRest_LORO_ExpandedSearch",
    ...
    out_json = output_dir / "pseudo_online_loro_expanded_search_results.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(overall, f, indent=2, ensure_ascii=False)

    out_csv = output_dir / "pseudo_online_loro_expanded_search_selected_configs.csv"
