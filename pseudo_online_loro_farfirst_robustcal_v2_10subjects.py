...
10 subjects, expanded window/step search, and a robust FAR-first decision layer.

Main changes relative to the FAR-first compact version:
- keep dual-threshold / hysteresis
- keep smoothing and consecutive-window logic
- keep arm_after_cue_sec cue-referenced
- keep refractory fixed at 0.0 for current one-decision-per-trial evaluation
- add probability calibration (CalibratedClassifierCV, sigmoid)
- choose configs with a robust FAR-first rule:
    * first prefer configs with mean inner-val FAR <= TARGET_MAX_INNER_VAL_FAR
      AND FAR std <= TARGET_MAX_INNER_VAL_FAR_STD
    * then prefer lower FAR variability, lower mean FAR
    * then prefer higher hit rate / balanced accuracy
    * if no config satisfies both FAR targets, progressively relax the rule

This version is intended to improve selection stability and reduce the
"inner validation looks good but outer test FAR explodes" pattern.

April 21, 2026, by XYan 
"""
...
ARM_AFTER_CUE_SEC_LIST = [0.0, 1.0, 1.25, 1.5]
...
def build_base_classifier(random_state=42):
    return Pipeline(
        [
...
def fit_calibrated_classifier(X_train, y_train, random_state=42):
    base = build_base_classifier(random_state=random_state)
...
def robust_far_first_sort_key(metrics):
    far = metrics["rest_false_alarm_rate"]
    far_std = metrics["rest_false_alarm_rate_std"]
    hit = metrics["mi_hit_rate"]
    hit_std = metrics["mi_hit_rate_std"]
    ...
    mean_far_feasible = far <= TARGET_MAX_INNER_VAL_FAR
    std_far_feasible = far_std <= TARGET_MAX_INNER_VAL_FAR_STD

    # Tier 0: mean FAR and FAR std both pass.
    # Tier 1: mean FAR passes but FAR std does not.
    # Tier 2: mean FAR fails.
    if mean_far_feasible and std_far_feasible:
        tier = 0
    elif mean_far_feasible:
        tier = 1
    else:
        tier = 2
...
def aggregate_inner_results(inner_results):
    ...
    aggregated.sort(
        key=lambda d: robust_far_first_sort_key(d["pseudo_online_inner_val_event_metrics"]),
    )
    return aggregated
...
def run_one_subject(data_dir, subject_id):
    print("=" * 80)
    print(f"Pseudo-online LORO robust FAR-first v2 | subject {subject_id}")
    print(f"RUNS                = {RUNS}")
    ...
    for test_run in RUNS:
        ...
        for val_run in dev_runs:
            ...
            for tmin, tmax in TRAIN_WINDOWS:
                ...
                clf = fit_calibrated_classifier(
                    X_train,
                    y_train,
                    random_state=42 + subject_id * 10 + test_run * 100 + val_run,
                )
        ...
        clf_final = fit_calibrated_classifier(
            X_dev,
            y_dev,
            random_state=42 + subject_id * 10 + test_run,
        )
        ...
        fold_summary = {
            ...
            "selection_status": "loro_inner_run_validation_far_first_robustcal_v2",
...
def main():
    ...
    for subject_id in SUBJECT_IDS:
        ...
        subj_json = output_dir / f"pseudo_online_loro_farfirst_robustcal_v2_subject{subject_id}.json"
            ...
            csv_rows.append(
                {
                    ...
                    "calibration_cv": best["calibration_cv"],
    ...
    overall = {
        "model": "PseudoOnline_Riemannian_TangentSpace_LR_MIvsRest_LORO_FARFirstRobustCalV2",
    ...
    overall["selection_policy"] = {
        "strategy": "far_first_robustcal_v2",
        "target_max_inner_val_far": TARGET_MAX_INNER_VAL_FAR,
        "target_max_inner_val_far_std": TARGET_MAX_INNER_VAL_FAR_STD,
        "fixed_refractory_sec": FIXED_REFRACTORY_SEC,
    ...
    out_json = output_dir / "pseudo_online_loro_farfirst_robustcal_v2_results.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(overall, f, indent=2, ensure_ascii=False)

    out_csv = output_dir / "pseudo_online_loro_farfirst_robustcal_v2_selected_configs.csv"
    if csv_rows:
