...
FAR-constrained model selection, and expanded window/step search.
...
3) Selection is FAR-constrained by default:
   - prefer configs whose validation false alarm rate <= MAX_ACCEPTABLE_FAR
   - if no config satisfies the FAR constraint, fall back to the best overall score
...
Compared with the previous repeated script:
- Keeps stricter causal latency by default: WINDOW_TIME_REFERENCE = "end"
- Opens win_sec / step_sec search space
- Adds FAR-constrained model selection and stores whether fallback was needed

April 16, 2026, FAR-constrained repeated-split version
"""
...
# FAR-constrained selection.
MAX_ACCEPTABLE_FAR = 0.15
MIN_ACCEPTABLE_HIT_RATE = None  # e.g. 0.60 if you want an additional constraint
MAX_LATENCY_CUE_SEC = None      # e.g. 2.0 if you want an additional constraint
...
def is_config_feasible(metrics):
    far = metrics["rest_false_alarm_rate"]
    hit = metrics["mi_hit_rate"]
    lat = metrics["mean_detection_latency_cue_sec"]

    if far is None or far > MAX_ACCEPTABLE_FAR:
        return False
    if MIN_ACCEPTABLE_HIT_RATE is not None:
        if hit is None or hit < MIN_ACCEPTABLE_HIT_RATE:
            return False
    if MAX_LATENCY_CUE_SEC is not None:
        if lat is None or lat > MAX_LATENCY_CUE_SEC:
            return False
    return True



def constrained_sort_key(metrics):
    """
    Higher is better.

    Feasible configs are always preferred over infeasible ones.
    Within feasible configs, prioritize:
    1) event balanced accuracy
    2) higher hit rate
    3) lower latency
    4) lower false alarm rate
    5) event F1

    If no feasible config exists, the same score still provides a sensible fallback.
    """
    far = metrics["rest_false_alarm_rate"]
    hit = metrics["mi_hit_rate"]
    lat = metrics["mean_detection_latency_cue_sec"]
    ...
    feasible = is_config_feasible(metrics)
...
def select_best_validation_result(val_search_results):
    feasible_results = [
        r for r in val_search_results if is_config_feasible(r["pseudo_online_val_event_metrics"])
    ]

    if len(feasible_results) > 0:
        feasible_results.sort(
            key=lambda d: constrained_sort_key(d["pseudo_online_val_event_metrics"]),
            reverse=True,
        )
        best_result = feasible_results[0]
        selection_status = "far_constrained"
    else:
        ...
        selection_status = "fallback_no_feasible_config"

    return best_result, selection_status, len(feasible_results)



def run_one_subject(data_dir, subject_id):
    print("=" * 80)
    print(f"Pseudo-online repeated FAR-constrained train/val/test | subject {subject_id}")
    print(f"RUNS                    = {RUNS}")
    ...
    print(f"MAX_ACCEPTABLE_FAR      = {MAX_ACCEPTABLE_FAR}")
    print(f"MIN_ACCEPTABLE_HIT_RATE = {MIN_ACCEPTABLE_HIT_RATE}")
    print(f"MAX_LATENCY_CUE_SEC     = {MAX_LATENCY_CUE_SEC}")
    print("=" * 80)
    ...
    for repeat_idx in range(N_REPEATS):
        ...
        for tmin, tmax in TRAIN_WINDOWS:
            ...
            for win_sec, step_sec in itertools.product(WIN_SEC_LIST, STEP_SEC_LIST):
                ...
                for smooth_k, threshold, min_consecutive in itertools.product(
                    ...
                    event_metrics["is_feasible"] = bool(is_config_feasible(event_metrics))
        ...
        best_val_result, selection_status, n_feasible = select_best_validation_result(val_search_results)
        ...
        repeat_summary = {
            ...
            "num_feasible_validation_configs": int(n_feasible),
        ...
        print(
            f"Repeat {repeat_idx + 1}: status={selection_status}, feasible={n_feasible}, "
    ...
    subject_summary = {
        ...
        "selection_constraints": {
            "max_acceptable_far": MAX_ACCEPTABLE_FAR,
            "min_acceptable_hit_rate": MIN_ACCEPTABLE_HIT_RATE,
            "max_latency_cue_sec": MAX_LATENCY_CUE_SEC,
        },
        "search_space": {
...
def summarize_overall(subject_results):
    ...
    for subj in subject_results:
        ...
        for rep in subj["repeat_results"]:
            ...
            if rep["selection_status"] == "far_constrained":
                total_far_constrained += 1
            else:
                total_fallback += 1
    ...
    return {
        "selection_status_totals": {
            "far_constrained": int(total_far_constrained),
            "fallback_no_feasible_config": int(total_fallback),
        },
...
def main():
    ...
    for subject_id in SUBJECT_IDS:
        ...
        per_subject_file = output_dir / f"pseudo_online_repeated_far_constrained_subject{subject_id}.json"
    ...
    overall_summary = {
        "model": "PseudoOnline_Riemannian_TangentSpace_LR_MIvsRest_RepeatedTrainValTest_FARConstrained",
    ...
    overall_file = output_dir / "pseudo_online_repeated_far_constrained_results.json"
