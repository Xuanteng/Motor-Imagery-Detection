...
10 subjects, expanded window/step search, and a FAR-first decision layer.

Main changes relative to the broader hysteresis/refractory search:
- keep dual-threshold / hysteresis
- keep smoothing and consecutive-window logic
- fix `arm_after_cue_sec` so it is applied using cue-referenced time
- remove refractory from the hyperparameter search and hold it fixed at 0.0
  because refractory does not materially affect one-decision-per-trial metrics
- choose the best config with a FAR-first validation rule:
    * first prefer configs whose inner-val FAR is <= TARGET_MAX_INNER_VAL_FAR
    * then prefer higher hit rate / balanced accuracy
    * if no config satisfies the FAR target, fall back to the lowest-FAR options

This version is intentionally more compact and more aligned with the current
pseudo-online objective of reducing false alarms before chasing accuracy.
...
# Timing / selection controls.
# Refractory is held fixed in this compact version because the current
# trial-level metrics depend almost entirely on the first trigger.
FIXED_REFRACTORY_SEC = 0.0

# These values are intentionally later than the original search because
# with tmin=0.5 and WINDOW_TIME_REFERENCE='end', very small values often do
# not change the admissible windows at all.
ARM_AFTER_CUE_SEC_LIST = [0.0, 1.0, 1.25, 1.5]

# FAR-first inner-validation target.
...
def far_first_sort_key(metrics, target_max_far=TARGET_MAX_INNER_VAL_FAR):
    ...
    feasible = far <= target_max_far
    excess_far = max(0.0, far - target_max_far)

    # Ascending sort:
    # 1) feasible configs first
    # 2) among infeasible configs, minimize FAR exceedance
    # 3) then maximize hit rate / balanced accuracy
    # 4) then prefer lower FAR and lower latency
    return (
...
def aggregate_inner_results(inner_results):
    ...
    aggregated.sort(
        key=lambda d: far_first_sort_key(d["pseudo_online_inner_val_event_metrics"])
    )
    return aggregated
...
def run_one_subject(data_dir, subject_id):
    print("=" * 80)
    print(f"Pseudo-online LORO FAR-first compact | subject {subject_id}")
    print(f"RUNS           = {RUNS}")
    ...
    print(f"REFRACTORY     = fixed {FIXED_REFRACTORY_SEC}")
    ...
    for test_run in RUNS:
        ...
        fold_summary = {
            ...
            "selection_status": "loro_inner_run_validation_far_first_compact",
...
def main():
    ...
    for subject_id in SUBJECT_IDS:
        ...
        subj_json = output_dir / f"pseudo_online_loro_farfirst_compact_subject{subject_id}.json"
    ...
    overall = {
        "model": "PseudoOnline_Riemannian_TangentSpace_LR_MIvsRest_LORO_FARFirstCompact",
    ...
    out_json = output_dir / "pseudo_online_loro_farfirst_compact_results.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(overall, f, indent=2, ensure_ascii=False)

    out_csv = output_dir / "pseudo_online_loro_farfirst_compact_selected_configs.csv"
