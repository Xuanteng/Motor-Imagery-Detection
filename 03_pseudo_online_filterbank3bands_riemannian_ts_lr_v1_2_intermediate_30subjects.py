...
# -*- coding: utf-8 -*-
"""
Pseudo-online MI-vs-Rest detection with 3-band Filter-bank Riemannian Tangent Space + Logistic Regression, intermediate decision-layer v1.2.

Purpose
-------
This script is a 3-band v1.2 revision of the filter-bank Riemannian benchmark.
It keeps the same model and evaluation logic as v1, but uses an intermediate
pseudo-online decision search space to balance hit rate and false alarms before expanding the dataset:
...
- intermediate decision candidates: moderate smoothing, high thresholds, min-consecutive 3/4, delayed arming
...
April 29, 2026, XYan
Revision v1.2 3-band: intermediate decision-layer search space; filter bank changed to 8-13, 13-20, 20-30 Hz.
"""
...
def run_one_subject(data_dir, subject_id):
    print("=" * 80)
    print(f"Pseudo-online 3-band FilterBank Riemannian TS-LR v1.2 | subject {subject_id}")
    print(f"RUNS               = {RUNS}")
    print(f"BANDS              = {BANDS}")
    ...
    for test_run in RUNS:
        ...
        fold_summary = {
            ...
            "selection_status": f"loro_inner_run_validation_filterbank_riemannian_ts_lr_v1_2_intermediate_3bands::{selection_meta['selection_mode']}",
...
def main():
    ...
    for subject_id in SUBJECT_IDS:
        ...
        subj_json = output_dir / f"pseudo_online_filterbank3bands_riemannian_ts_lr_v1_2_intermediate_30subjects_subject{subject_id}.json"
    ...
    overall = {
        "model": "PseudoOnline_FilterBank3Bands_Riemannian_TangentSpace_LR_MIvsRest_LORO_BalancedRobustCalV1_2Intermediate",
    ...
    overall["selection_policy"] = {
        "strategy": "balanced_robust_filterbank3bands_riemannian_ts_lr_v1_2_intermediate",
        "target_max_inner_val_far": TARGET_MAX_INNER_VAL_FAR,
        "target_max_inner_val_far_std": TARGET_MAX_INNER_VAL_FAR_STD,
        "target_min_inner_val_hit": TARGET_MIN_INNER_VAL_HIT,
        "fixed_refractory_sec": FIXED_REFRACTORY_SEC,
    ...
    out_json = output_dir / f"pseudo_online_filterbank3bands_riemannian_ts_lr_v1_2_intermediate_30subjects_{suffix}.json"
    with open(out_json, "w", encoding="utf-8") as f:
        json.dump(overall, f, indent=2, ensure_ascii=False)

    out_csv = output_dir / f"pseudo_online_filterbank3bands_riemannian_ts_lr_v1_2_intermediate_30subjects_{suffix}_selected_configs.csv"
    if csv_rows:
    ...
    print("=" * 80)
    print("Finished 3-band FilterBank Riemannian TS-LR intermediate v1.2 benchmark")
